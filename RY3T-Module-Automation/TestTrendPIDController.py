...
if __name__ == "__main__":
    # Konfigurationsdatei
    config_file = "test_config.json"  # oder "config.json" wenn gewünscht
    
    # Controller mit Config-Datei und optional MQTT initialisieren
    controller = MaxTempDeltaTController(
        config_file=config_file,
        mqtt_broker="localhost"  # Optional - kann auch in config.json stehen
    )
    
    # System starten (aktiviert Pumpen wenn MQTT aktiv)
    controller.start_control()
    
    # Zeige aktuelle Konfiguration
    print("\nAktuelle Konfiguration:")
    print(f"- Primär Solltemperatur: {controller.primary_setpoint}°C")
    print(f"- Harte Regelung ab: +{controller.hard_control_threshold}°C")
